...
if len(os.listdir(opt.netd_save_path)) > 0 and len(os.listdir(opt.netg_save_path)) > 0:
    opt.prev_model = True
    print(f"Previous model detected.")
else:
    opt.prev_model = False
    print(f"No prev model detected.")
...
def train(**kwargs):
    ...
    # 初始化网络
    netg, netd = CNN_Generator(opt), CNN_Discriminator(opt)
    # 判断网络是否有权重数值
    # ### storage存储
    ...
    # os.mkdir(opt.netg_save_path, exist_ok=True)
    # os.mkdir(opt.netd_save_path, exist_ok=True)
    if opt.prev_model and opt.netg_save_path:
        netg.load_state_dict(torch.load(f=opt.netg_save_path, map_location=map_location))
    if opt.prev_model and opt.netd_save_path:
        netd.load_state_dict(torch.load(f=opt.netd_save_path, map_location=map_location))
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.save_every == 0:
            ...
            torch.save(netd.state_dict(),  opt.netd_save_path + '/netd_{0}.pth'.format(epoch))
            torch.save(netg.state_dict(),  opt.netg_save_path + '/netg_{0}.pth'.format(epoch))
...
def generate(**kwargs):
    ...
    # 加载训练好的权重数据
    netg, netd = CNN_Generator(opt).eval(), CNN_Discriminator(opt).eval()
    #  两个参数返回第一个
    map_location = lambda storage, loc: storage

    # opt.netd_save_path等参数有待修改
    netd.load_state_dict(torch.load(opt.netd_load_file, map_location=map_location), False)
    netg.load_state_dict(torch.load(opt.netg_load_file, map_location=map_location), False)
    netd.to(device)
    netg.to(device)
